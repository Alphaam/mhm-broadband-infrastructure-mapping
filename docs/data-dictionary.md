...
| File | Purpose |
|---|---|
| `service-area-counties.geojson` | Drives the county half of the search/zoom control (`components/map/place-search.tsx`). County name field is `CNTY_NM` (real value, e.g. "Atascosa") — `place-search.tsx` passes that explicitly to `countiesFromFeatureCollection()`, since the default is the more generic `NAME`. 74 counties. |
| `service-area-regions.geojson` | Drives the region half of the same control. **Not independently sourced** — generated by dissolving `service-area-counties.geojson` on the county→region crosswalk in `scripts/data/regions-crosswalk.csv` (HR&A's "MHM County_Region Crosswalk.csv", with "De Witt" normalized to "DeWitt" to match `CNTY_NM`). Region field is `REGION` (single-letter id, e.g. "A"). Regenerate with `./scripts/data/build-regions.sh` if the county boundaries or crosswalk change; it also writes a real `.shp` to `data/raw/service-area-regions.shp` for GIS use outside the app. 12 regions. |
